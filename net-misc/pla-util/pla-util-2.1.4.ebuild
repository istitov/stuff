...
src_compile() {
	...
	# 2.1.4 selects net/src/<OS> and the link flags from the OS external,
	# which gprbuild also reads from the environment; pin the Linux variant.
	# verified 2026-09-26
	gprbuild -p -P pla_util.gpr -XOS=linux -j$(get_makeopts_jobs) \
