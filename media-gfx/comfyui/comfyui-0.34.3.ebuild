...
# comfy_aimdo has four unguarded module-load imports. comfy_kitchen's guarded
# import only degrades FP messages; Flux/Lumina/Z-Image RoPE still needs it at
# sampling. The GGUF freeze entry belongs to a custom node, not core.
RDEPEND="${PYTHON_DEPS}
	...
	')
	templates? ( ~dev-python/comfyui-workflow-templates-0.11.54[${PYTHON_SINGLE_USEDEP}] )
	extra? (
