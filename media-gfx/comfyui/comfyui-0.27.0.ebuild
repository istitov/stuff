...
RDEPEND="${PYTHON_DEPS}
	...
	~dev-python/comfyui-frontend-package-1.45.20[${PYTHON_SINGLE_USEDEP}]
	~dev-python/comfyui-embedded-docs-0.5.6[${PYTHON_SINGLE_USEDEP}]
	$(python_gen_cond_dep '
	...
	')
	templates? ( ~dev-python/comfyui-workflow-templates-0.11.1[${PYTHON_SINGLE_USEDEP}] )
	extra? (
...
#
# 0.27.0 dropped glfw from requirements (no remaining consumer). Its only
# PyOpenGL consumer, comfy_extras/nodes_glsl.py, now imports comfy_angle first
# to pre-load the ANGLE EGL/GLES runtime before PyOpenGL. comfy-angle (a
# Comfy-Org binary wheel) is not packaged here yet, so the GLSL shader nodes
# stay unavailable regardless of USE=opengl -- init_builtin_extra_nodes()
# catches the missing per-module import and skips just that node file (no
# startup crash). USE=opengl still installs PyOpenGL for when comfy-angle
