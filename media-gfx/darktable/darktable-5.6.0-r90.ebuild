...
IUSE="X ai avif colord cpu_flags_x86_avx cpu_flags_x86_sse3 cups doc gamepad geolocation keyring gphoto2 graphicsmagick heif jpeg2k jpegxl kwallet lto lua midi opencl openmp openexr test tools wayland webp
	${LANGS// / l10n_}"
...
pkg_postinst() {
	...
	if use ai; then
		elog
		elog "The AI subsystem (USE=ai) ships no models. Neural restore,"
		elog "raw/image denoise, upscaling and the AI object-mask models are"
		elog "downloaded on demand from within darktable (Preferences), from the"
		elog "darktable-org/darktable-ai model repository. Inference runs on the"
		elog "system sci-libs/onnxruntime."
	fi
