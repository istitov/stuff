...
# This revision is fundamentally different from pf-sources-6.7_p7-r1.
# Instead of fetching pf-kernel/codeberg's GA-only sourcetree (Linux 6.7.0
# + pf patchset, no linux-stable updates), it builds on top of vanilla
# 6.7.tar.xz + Gentoo genpatches (sourced per-patch from alicef's live
# trunk dir, since release tarballs were never cut for the 6.7 branch),
...
# The slot's pretend version stays "6.7_p7" so this ebuild remains
...
# linux-stable backport chain (1000_…1011_) covers 6.7.1 through 6.7.12.
GENPATCHES_STABLE=(
...
# Non-stable additions; both 5010 and 5020 are excluded per per-branch
# judgment (see header comment). 6.7 trunk has no 1500_XATTR_USER_PREFIX
# patch — that addition was dropped between 6.6 and 6.7.
GENPATCHES_EXTRA=(
...
src_prepare() {
	...
	eapply "${FILESDIR}/pf-curated-6.7"/*.patch
	default
}
