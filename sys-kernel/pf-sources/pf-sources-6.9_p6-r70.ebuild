...
# This revision is fundamentally different from pf-sources-6.9_p6-r1.
# Instead of fetching pf-kernel/codeberg's GA-only sourcetree (Linux 6.9.0
# + pf patchset, no linux-stable updates), it builds on top of vanilla
# 6.9.tar.xz + Gentoo genpatches (sourced per-patch from alicef's live
# trunk dir, since release tarballs were never cut for the 6.9 branch),
...
# The slot's pretend version stays "6.9_p6" so this ebuild remains
...
# linux-stable backport chain (1000_…1011_) covers 6.9.1 through 6.9.12.
GENPATCHES_STABLE=(
...
# Non-stable additions; all 5xxx excluded per per-branch judgment (see
# header comment).
GENPATCHES_EXTRA=(
...
src_prepare() {
	...
	eapply "${FILESDIR}/pf-curated-6.9"/*.patch
	default
}
...
pkg_postinst() {
	...
	elog "It tracks linux-stable through 6.9.12 (full upstream coverage — 6.9"
	elog "ended at .12) via alicef's genpatches trunk AND keeps a curated"
	elog "subset of natalenko's pf-kernel delta on top. 6.9 is an EOL non-LTS"
	...
	elog "  * Most 'minor fixes' pf carries are now in linux-stable's 6.9.X"
	...
	elog "    pf-flavored vanilla; section anchors are off in vanilla 6.9"
	elog "    from kernel.org. Dropped — pf's own ISA Kconfig naturally lands"
	elog "    in the curated subset since stable backports don't touch"
	elog "    arch/x86/Kconfig.cpu on this slot."
	...
	elog "pf-sources-6.9_p6-r1 instead — it stays GA-frozen and ships"
	elog "natalenko's patchset verbatim, at the cost of missing all twelve"
	elog "linux-stable releases (6.9.1-6.9.12) that this revision applies."
	elog ""
