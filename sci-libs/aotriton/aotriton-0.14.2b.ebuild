...
EGIT_COMMIT="${PV}"
EGIT_SUBMODULES=( '*' )
# FlyDSL kernel sources (third_party/flydsl-kernel.txt); upstream clones them
# at configure time.
FLYDSL_KERNEL_PV="0.3.1"
SRC_URI="
	https://github.com/ROCm/FlyDSL/archive/refs/tags/v${FLYDSL_KERNEL_PV}.tar.gz
		-> flydsl-${FLYDSL_KERNEL_PV}.gh.tar.gz
"
...
# Match aotriton-bin's 6.4 floor and next-major cap. verified 2026-09-26
RDEPEND="
...
BDEPEND="
	...
	')
	$(python_gen_any_dep '
		dev-python/flydsl[${PYTHON_SINGLE_USEDEP}]
	')
"

PATCHES=(
	"${FILESDIR}/${PN}-0.13b-include-iostream.patch"
	"${FILESDIR}/${P}-system-flydsl.patch"
)
...
python_check_deps() {
	python_has_version "dev-python/filelock[${PYTHON_USEDEP}]" &&
		...
		python_has_version "dev-python/flydsl[${PYTHON_SINGLE_USEDEP}]"
}
...
src_prepare() {
	...
	# Expose declared build dependencies and the system flydsl in the
	# disposable venv; install the local Triton wheel into it below.
	[[ $(grep -c 'execute_process(COMMAND.*-m venv "${VENV_DIR}")' CMakeLists.txt) == 1 &&
		$(grep -c -- '-m pip install "${CMAKE_CURRENT_LIST_DIR}"' CMakeLists.txt) == 1 ]] ||
		die "venv setup lines moved; re-audit the sed below"
	sed -i \
...
src_configure() {
	...
	# Leave ROCM_PATH unset here: hip-config-amd.cmake derives its clang root
	# and device libraries from it, and the flydsl toolkit tree set below
	# for the kernel build would break hip::host. verified 2026-09-26
	...
	local mycmakeargs=(
		...
		-DAOTRITON_USE_SYSTEM_FLYDSL=ON
		-DAOTRITON_FLYDSL_KERNEL_ROOT="${WORKDIR}/FlyDSL-${FLYDSL_KERNEL_PV}"
...
src_install() {
	local -x PIP_NO_INDEX=1
	# The kernels are compiled here. flyc_compile needs a ROCm root holding
	# llvm/bin/ld.lld and llvm/bin/llvm-readelf: dev-python/flydsl's toolkit.
	local -x ROCM_PATH="${EPREFIX}/usr/lib/flydsl/rocm"
	cmake_build install
