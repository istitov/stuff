...
# Cargo.lock pulls crates from git (tikv/raft-rs at a pinned rev) that are not
# on crates.io, so the dependency graph is fetched by cargo at build time
# rather than vendored via CRATES. Hence network access + the non-determinism
# marker.
PROPERTIES="live"
