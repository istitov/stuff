...
IUSE="aqua colord cups debug doc examples +introspection packagekit test vim-syntax wayland X xinerama appmenu overlay"
REQUIRED_USE="
	|| ( aqua wayland X )
	xinerama? ( X )"

KEYWORDS="~alpha ~amd64 ~arm ~hppa ~ia64 ~ppc ~ppc64 ~sh ~sparc ~x86 ~amd64-fbsd ~x86-fbsd ~x86-freebsd ~x86-interix ~amd64-linux ~x86-linux ~ppc-macos ~x86-macos ~sparc-solaris ~sparc64-solaris ~x64-solaris ~x86-solaris"
...
src_prepare() {
	...
	if use appmenu;then
	epatch "${FILESDIR}"/appmenu-gtk3/${PV}/*.patch*
	fi
