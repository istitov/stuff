...
# requests moved to an extra in 1.45.0: HTTP goes through the urllib3
# transport, and requests is needed only for a caller-supplied Session or a
# credential provider.
RDEPEND="
	...
	~dev-python/opentelemetry-exporter-otlp-common-${PV}[${PYTHON_USEDEP}]
...
EPYTEST_PLUGINS=()
# These suites mock HTTP with the unpackaged mocket.
EPYTEST_IGNORE=(
	tests/metrics/test_otlp_metrics_exporter.py
	tests/test_proto_log_exporter.py
	tests/test_proto_span_exporter.py
)
...
pkg_postinst() {
	optfeature "requests Sessions and credential providers" dev-python/requests
}
