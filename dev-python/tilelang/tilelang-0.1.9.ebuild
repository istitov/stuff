...
# verified 2026-05-07
# Retain upstream's tvm-ffi >=0.1.2,<0.2 range; consumers may pin tighter.
RDEPEND="
	...
	$(python_gen_cond_dep '
		>=dev-python/apache-tvm-ffi-0.1.2[${PYTHON_USEDEP}]
...
python_install_all() {
	...
	if use cuda; then
		...
		done < <(find "${ED}" -name 'libtilelang.so' -print0)
	fi
}
