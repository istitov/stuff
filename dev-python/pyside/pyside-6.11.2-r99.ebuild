...
# Fork of ::gentoo's dev-python/pyside that adds a `datavis`
# USE flag wiring up PySide6.QtDataVisualization. ::gentoo dropped
# QtDataVisualization during the Qt6 import (it was deprecated by
# upstream as of Qt 6.6 in favour of QtGraphs), but
# sci-physics/sasview's Shape2SAS calculator unconditionally imports
# from PySide6.QtDataVisualization. Pairs with this overlay's
# dev-qt/qtdatavis3d ebuild.
...
if [[ ${PV} == *.9999 ]]; then
	...
	# (overlay-only) and dev-qt/qtgraphs (::gentoo) at 6.11.1 are ~amd64-only,
...
# 6.11.2 dropped the need for the -fix-llvm-version patch: upstream widened
# the LLVM_VERSION regex in ShibokenGeneratorSetup.cmake to
# "\.[0-9]+\.[0-9]+.*$", which strips any suffix and yields the bare LLVM
# major (e.g. 22) that the "#if LLVM_VERSION >= 22" guards require -- the
# same result the patch forced via ${LLVM_VERSION_MAJOR}.
PATCHES=(
