...
# 0.22.2 in 1.6.0) to the buoyant-data delta-kernel-rs fork pinned to a branch
# commit, so these two move out of CRATES into GIT_CRATES (fetched + extracted).
# The eclass's generated [patch] does NOT redirect a branch-pinned + package-
# renamed git dep under the maturin/offline build, so src_prepare also repoints
# the workspace dep at the extracted tree. verified 2026-06-25.
...
src_prepare() {
	...
	grep -qF "${from}" "${ws_toml}" ||
		die "delta_kernel git dep line not found — upstream changed the pin form, re-audit"
	sed -i -e "s|${from}|${to}|" "${ws_toml}" || die
