# Copyright 1999-2026 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

DISTUTILS_EXT=1
DISTUTILS_USE_PEP517=maturin
PYTHON_COMPAT=( python3_{12..14} )

KERNEL_COMMIT="ab66589067127b81c87690370e9b2c6e5129bcdf"

CRATES="
	RustyXML@0.3.0
	adler2@2.0.1
	aes@0.8.4
	ahash@0.8.12
	aho-corasick@1.1.5
	alloc-no-stdlib@2.0.4
	alloc-no-stdlib@3.0.0
	alloc-stdlib@0.2.2
	alloc-stdlib@0.3.0
	allocator-api2@0.2.21
	android_system_properties@0.1.6
	anes@0.1.6
	anstream@1.0.0
	anstyle@1.0.14
	anstyle-parse@1.0.0
	anstyle-query@1.1.5
	anstyle-wincon@3.0.11
	anyhow@1.0.104
	approx@0.5.1
	ar_archive_writer@0.5.3
	arc-swap@1.9.2
	arrayvec@0.7.8
	arrow@59.3.0
	arrow-arith@59.3.0
	arrow-array@59.3.0
	arrow-buffer@59.3.0
	arrow-cast@59.3.0
	arrow-csv@59.3.0
	arrow-data@59.3.0
	arrow-ipc@59.3.0
	arrow-json@59.3.0
	arrow-ord@59.3.0
	arrow-pyarrow@59.3.0
	arrow-row@59.3.0
	arrow-schema@59.3.0
	arrow-select@59.3.0
	arrow-string@59.3.0
	assert-json-diff@2.0.2
	async-channel@1.9.0
	async-channel@2.5.0
	async-compression@0.4.48
	async-executor@1.14.0
	async-ffi@0.5.1
	async-global-executor@2.4.1
	async-io@2.6.0
	async-lock@3.4.2
	async-object-pool@0.2.0
	async-std@1.13.2
	async-stream@0.3.6
	async-stream-impl@0.3.6
	async-task@4.7.1
	async-trait@0.1.92
	asyncband@0.6.7
	asyncband@0.7.2
	atoi@2.0.0
	atomic-waker@1.1.2
	autocfg@1.5.1
	awaitable@0.4.0
	awaitable-error@0.1.0
	aws-config@1.12.0
	aws-credential-types@1.3.0
	aws-lc-rs@1.18.1
	aws-lc-sys@0.45.0
	aws-runtime@1.9.4
	aws-sdk-glue@1.167.0
	aws-sdk-sso@1.111.0
	aws-sdk-ssooidc@1.113.0
	aws-sdk-sts@1.116.0
	aws-sigv4@1.5.3
	aws-smithy-async@1.3.0
	aws-smithy-http@0.64.0
	aws-smithy-http-client@1.4.2
	aws-smithy-json@0.63.1
	aws-smithy-observability@0.3.0
	aws-smithy-query@0.62.1
	aws-smithy-runtime@1.14.2
	aws-smithy-runtime-api@1.16.2
	aws-smithy-runtime-api-macros@1.1.0
	aws-smithy-schema@0.2.1
	aws-smithy-types@1.6.4
	aws-smithy-xml@0.62.1
	aws-types@1.6.0
	azure_core@0.21.0
	azure_storage@0.21.0
	azure_storage_blobs@0.21.0
	azure_svc_blobstorage@0.21.0
	base64@0.13.1
	base64@0.22.1
	base64@0.23.1
	base64-simd@0.8.0
	base64ct@1.8.3
	bigdecimal@0.4.10
	bincode@1.3.3
	bitflags@1.3.2
	bitflags@2.13.2
	blake2@0.10.6
	blake3@1.8.7
	block-buffer@0.10.4
	block-buffer@0.12.1
	block-padding@0.3.3
	blocking@1.7.0
	brotli@8.0.4
	brotli-decompressor@5.0.3
	brotli-decompressor@6.0.0
	bstr@1.13.1
	bumpalo@3.20.3
	bytemuck@1.25.2
	byteorder@1.5.0
	bytes@1.12.1
	bytes-utils@0.1.4
	bzip2@0.6.1
	cast@0.3.0
	cbc@0.1.2
	cc@1.4.7
	cfg-if@1.0.5
	cfg_aliases@0.2.2
	chacha20@0.10.2
	chrono@0.4.45
	chrono-tz@0.10.4
	ciborium@0.2.2
	ciborium-io@0.2.2
	ciborium-ll@0.2.2
	cipher@0.4.4
	clap@4.6.7
	clap_builder@4.6.7
	clap_derive@4.6.7
	clap_lex@1.1.1
	cmake@0.1.58
	cmov@0.5.4
	colorchoice@1.0.5
	colored@3.1.1
	combine@4.6.8
	comfy-table@7.2.2
	compression-codecs@0.4.43
	compression-core@0.4.33
	concurrent-queue@2.5.0
	concurrent_arena@0.1.11
	condtype@1.3.0
	const-oid@0.9.6
	const-oid@0.10.2
	const-random@0.1.18
	const-random-macro@0.1.16
	const-str@1.1.0
	const_panic@0.2.17
	constant_time_eq@0.4.2
	convert_case@0.9.0
	core-foundation@0.9.4
	core-foundation@0.10.1
	core-foundation-sys@0.8.7
	core_affinity@0.8.3
	core_detect@1.0.0
	countio@0.3.0
	cpufeatures@0.2.17
	cpufeatures@0.3.1
	crc@3.4.0
	crc-catalog@2.5.0
	crc32c@0.6.8
	crc32fast@1.5.2
	criterion@0.5.1
	criterion-plot@0.5.0
	crossbeam-channel@0.5.17
	crossbeam-deque@0.8.8
	crossbeam-epoch@0.9.21
	crossbeam-utils@0.8.23
	crunchy@0.2.4
	crypto-common@0.1.7
	crypto-common@0.2.2
	csv@1.4.0
	csv-core@0.1.13
	ctor@0.10.1
	ctor@0.13.1
	ctor@1.0.13
	ctor-proc-macro@0.0.13
	ctr@0.9.2
	ctutils@0.4.2
	darling@0.20.11
	darling_core@0.20.11
	darling_macro@0.20.11
	dashmap@6.2.1
	datafusion@55.1.0
	datafusion-catalog@55.1.0
	datafusion-catalog-listing@55.1.0
	datafusion-common@55.1.0
	datafusion-common-runtime@55.1.0
	datafusion-datasource@55.1.0
	datafusion-datasource-arrow@55.1.0
	datafusion-datasource-csv@55.1.0
	datafusion-datasource-json@55.1.0
	datafusion-datasource-parquet@55.1.0
	datafusion-doc@55.1.0
	datafusion-execution@55.1.0
	datafusion-expr@55.1.0
	datafusion-expr-common@55.1.0
	datafusion-ffi@55.1.0
	datafusion-functions@55.1.0
	datafusion-functions-aggregate@55.1.0
	datafusion-functions-aggregate-common@55.1.0
	datafusion-functions-nested@55.1.0
	datafusion-functions-table@55.1.0
	datafusion-functions-window@55.1.0
	datafusion-functions-window-common@55.1.0
	datafusion-macros@55.1.0
	datafusion-optimizer@55.1.0
	datafusion-physical-expr@55.1.0
	datafusion-physical-expr-adapter@55.1.0
	datafusion-physical-expr-common@55.1.0
	datafusion-physical-optimizer@55.1.0
	datafusion-physical-plan@55.1.0
	datafusion-proto@55.1.0
	datafusion-proto-common@55.1.0
	datafusion-proto-models@55.1.0
	datafusion-pruning@55.1.0
	datafusion-session@55.1.0
	datafusion-sql@55.1.0
	datasketches@0.3.0
	defmt@1.1.1
	defmt-macros@1.1.1
	defmt-parser@1.0.0
	der@0.7.10
	deranged@0.5.8
	derive_destructure2@0.1.3
	des@0.8.1
	diff@0.1.13
	digest@0.10.7
	digest@0.11.3
	dirs@6.0.0
	dirs-sys@0.5.0
	displaydoc@0.2.7
	divan@0.1.21
	divan-macros@0.1.21
	dlv-list@0.5.2
	dns-lookup@3.0.1
	dotenvy@0.15.7
	dtor@0.8.1
	dtor-proc-macro@0.0.13
	dunce@1.0.5
	dyn-clone@1.0.20
	either@1.18.0
	encoding_rs@0.8.41
	env_filter@2.0.0
	env_home@0.1.0
	env_logger@0.10.2
	env_logger@0.11.11
	equivalent@1.0.2
	errno@0.3.14
	event-listener@2.5.3
	event-listener@5.4.2
	event-listener-strategy@0.5.4
	fastant@0.1.11
	fastpool@1.1.2
	fastrand@1.9.0
	fastrand@2.5.0
	find-msvc-tools@0.1.13
	fixedbitset@0.5.7
	flatbuffers@25.12.19
	flate2@1.1.10
	fnv@1.0.7
	foldhash@0.1.5
	foldhash@0.2.0
	foreign-types@0.3.2
	foreign-types-shared@0.1.1
	form_urlencoded@1.2.2
	foyer@0.22.6
	foyer-common@0.22.6
	foyer-intrusive-collections@0.10.0-dev
	foyer-memory@0.22.6
	foyer-storage@0.22.6
	foyer-tokio@0.22.6
	fs4@0.13.1
	fs_extra@1.3.0
	futures@0.3.34
	futures-channel@0.3.34
	futures-core@0.3.34
	futures-executor@0.3.34
	futures-io@0.3.34
	futures-lite@1.13.0
	futures-lite@2.6.1
	futures-macro@0.3.34
	futures-rustls@0.26.0
	futures-sink@0.3.34
	futures-task@0.3.34
	futures-timer@3.0.4
	futures-util@0.3.34
	g2gen@1.2.2
	g2p@1.2.2
	g2poly@1.2.2
	gearhash@0.1.4
	generic-array@0.14.7
	getrandom@0.1.16
	getrandom@0.2.17
	getrandom@0.3.4
	getrandom@0.4.3
	git-version@0.3.9
	git-version-macro@0.3.9
	glob@0.3.4
	gloo-timers@0.3.0
	h2@0.4.19
	half@2.7.1
	hashbrown@0.14.5
	hashbrown@0.15.5
	hashbrown@0.17.1
	hdfs-native@0.13.5
	hdfs-native-object-store@0.16.0
	headers@0.4.1
	headers-core@0.3.0
	heapify@0.2.0
	heck@0.5.0
	hermit-abi@0.5.3
	hex@0.4.3
	hf-xet@1.6.0
	hmac@0.12.1
	hmac@0.13.0
	http@0.2.12
	http@1.5.0
	http-body@0.4.6
	http-body@1.1.0
	http-body-util@0.1.5
	http-types@2.12.0
	httparse@1.10.1
	httpdate@1.0.3
	httpmock@0.8.3
	humantime@2.4.0
	hybrid-array@0.4.15
	hyper@1.11.1
	hyper-rustls@0.27.10
	hyper-tls@0.6.0
	hyper-util@0.1.20
	iana-time-zone@0.1.65
	iana-time-zone-haiku@0.1.2
	icu_collections@2.3.0
	icu_locale_core@2.3.0
	icu_normalizer@2.3.0
	icu_normalizer_data@2.3.0
	icu_properties@2.3.0
	icu_properties_data@2.3.0
	icu_provider@2.3.1
	ident_case@1.0.1
	idna@1.1.0
	idna_adapter@1.2.2
	indexmap@2.14.2
	infer@0.2.3
	inout@0.1.4
	instant@0.1.13
	io-uring@0.7.15
	ipnet@2.12.2
	is-terminal@0.4.17
	is_terminal_polyfill@1.70.2
	itertools@0.10.5
	itertools@0.14.0
	itertools@0.15.0
	itoa@1.0.18
	jiff@0.2.37
	jiff-core@0.1.1
	jiff-static@0.2.37
	jiff-tzdb@0.1.8
	jiff-tzdb-platform@0.1.3
	jni@0.22.4
	jni-macros@0.22.4
	jni-sys@0.4.1
	jni-sys-macros@0.4.1
	jobserver@0.1.35
	js-sys@0.3.105
	konst@0.4.3
	konst_proc_macros@0.4.1
	kv-log-macro@1.0.7
	lazy-regex@3.6.1
	lazy-regex-proc_macros@3.6.1
	lazy_static@1.5.0
	lexical-core@1.0.6
	lexical-parse-float@1.0.6
	lexical-parse-integer@1.0.6
	lexical-util@1.0.7
	lexical-write-float@1.0.6
	lexical-write-integer@1.0.6
	libbz2-rs-sys@0.2.5
	libc@0.2.189
	libloading@0.9.0
	liblzma@0.4.8
	liblzma-sys@0.4.9
	libm@0.2.16
	libmimalloc-sys@0.1.49
	libredox@0.1.24
	link-section@0.2.1
	link-section@0.13.1
	link-section@0.19.3
	linktime-proc-macro@0.1.0
	linktime-proc-macro@0.2.3
	linux-raw-sys@0.12.1
	litemap@0.8.3
	lock_api@0.4.14
	log@0.4.34
	lru-slab@0.1.3
	lz4@1.28.1
	lz4-sys@1.11.1+lz4-1.10.0
	lz4_flex@0.13.1
	lz4_flex@0.14.0
	maplit@1.0.2
	matchers@0.2.0
	matrixmultiply@0.3.11
	md-5@0.10.6
	md-5@0.11.0
	mea@0.6.7
	memchr@2.8.3
	memoffset@0.9.1
	mimalloc@0.1.52
	mime@0.3.17
	miniz_oxide@0.9.1
	mio@1.2.3
	mixtrics@0.2.5
	mockito@1.7.2
	moka@0.12.16
	more-asserts@0.3.1
	multiversion@0.9.0
	multiversion-macros@0.9.0
	multiversion_no_op@1.0.0
	native-tls@0.2.18
	ndarray@0.17.2
	ntapi@0.4.3
	nu-ansi-term@0.50.3
	num-bigint@0.4.8
	num-bigint@0.5.1
	num-bigint-dig@0.8.6
	num-complex@0.4.6
	num-conv@0.2.2
	num-derive@0.5.1
	num-integer@0.1.47
	num-iter@0.1.46
	num-traits@0.2.19
	num_cpus@1.17.0
	numpy@0.29.0
	objc2-core-foundation@0.3.2
	objc2-io-kit@0.3.2
	objc2-system-configuration@0.3.2
	object@0.39.1
	object_store@0.13.2
	object_store_opendal@0.56.0
	once_cell@1.21.4
	once_cell_polyfill@1.70.2
	oneshot@0.1.13
	oorandom@11.1.5
	opendal@0.56.0
	opendal-core@0.56.0
	opendal-service-azblob@0.56.0
	opendal-service-azdls@0.56.0
	opendal-service-azure-common@0.56.0
	opendal-service-b2@0.56.0
	opendal-service-cos@0.56.0
	opendal-service-fs@0.56.0
	opendal-service-ftp@0.56.0
	opendal-service-gcs@0.56.0
	opendal-service-hf@0.56.0
	opendal-service-obs@0.56.0
	opendal-service-oss@0.56.0
	opendal-service-s3@0.56.0
	opendal-service-sftp@0.56.0
	opendal-service-swift@0.56.0
	opendal-service-tos@0.56.0
	opendal-service-webdav@0.56.0
	opendal-service-webhdfs@0.56.0
	openssh@0.11.6
	openssh-sftp-client@0.15.9
	openssh-sftp-client-lowlevel@0.7.2
	openssh-sftp-error@0.5.1
	openssh-sftp-protocol@0.24.2
	openssh-sftp-protocol-error@0.1.1
	openssl@0.10.81
	openssl-macros@0.1.1
	openssl-probe@0.2.1
	openssl-sys@0.9.117
	opentelemetry@0.31.0
	opentelemetry-http@0.31.0
	opentelemetry-otlp@0.31.1
	opentelemetry-proto@0.31.0
	opentelemetry_sdk@0.31.0
	option-ext@0.2.0
	ordered-multimap@0.7.3
	os_str_bytes@6.6.1
	outref@0.5.2
	parking@2.2.1
	parking_lot@0.12.5
	parking_lot_core@0.9.12
	parquet@59.3.0
	parquet-variant@59.3.0
	parquet-variant-compute@59.3.0
	parquet-variant-json@59.3.0
	paste@1.0.15
	path-tree@0.8.3
	pbkdf2@0.12.2
	pem@4.0.0
	pem-rfc7468@0.7.0
	percent-encoding@2.3.2
	percent-encoding-rfc3986@0.1.3
	petgraph@0.8.3
	phf@0.12.1
	phf_shared@0.12.1
	pin-project@1.1.13
	pin-project-internal@1.1.13
	pin-project-lite@0.2.17
	pin-utils@0.1.0
	piper@0.2.5
	pkcs1@0.7.5
	pkcs5@0.7.1
	pkcs8@0.10.2
	pkg-config@0.3.34
	plain@0.2.3
	plotters@0.3.7
	plotters-backend@0.3.7
	plotters-svg@0.3.7
	polling@3.11.0
	portable-atomic@1.15.0
	portable-atomic-util@0.2.8
	potential_utf@0.1.6
	powerfmt@0.2.0
	ppv-lite86@0.2.21
	pretty_assertions@1.4.1
	pretty_env_logger@0.5.0
	proc-macro-crate@3.5.0
	proc-macro-error-attr2@2.0.0
	proc-macro-error2@2.0.1
	proc-macro2@1.0.107
	prost@0.14.4
	prost-derive@0.14.4
	prost-types@0.14.4
	psm@0.1.32
	pyo3@0.29.2
	pyo3-arrow@0.19.0
	pyo3-build-config@0.29.2
	pyo3-ffi@0.29.2
	pyo3-macros@0.29.2
	pyo3-macros-backend@0.29.2
	quick-xml@0.31.0
	quick-xml@0.38.4
	quick-xml@0.39.4
	quick-xml@0.41.0
	quinn@0.11.12
	quinn-proto@0.11.18
	quinn-udp@0.5.15
	quote@1.0.47
	r-efi@5.3.0
	r-efi@6.0.0
	rand@0.7.3
	rand@0.8.8
	rand@0.9.5
	rand@0.10.3
	rand_chacha@0.2.2
	rand_chacha@0.3.1
	rand_chacha@0.9.0
	rand_core@0.5.1
	rand_core@0.6.4
	rand_core@0.9.5
	rand_core@0.10.1
	rand_hc@0.2.0
	rand_pcg@0.10.2
	rawpointer@0.2.1
	rayon@1.12.0
	rayon-core@1.13.0
	recursive@0.1.1
	recursive-proc-macro-impl@0.1.1
	redb@3.1.3
	redox_syscall@0.5.18
	redox_syscall@0.9.4
	redox_users@0.5.3
	regex@1.13.1
	regex-automata@0.4.18
	regex-lite@0.1.9
	regex-syntax@0.8.11
	relative-path@1.9.3
	reqsign-aliyun-oss@3.1.5
	reqsign-aws-core@3.1.1
	reqsign-aws-v4@3.3.0
	reqsign-azure-storage@3.2.1
	reqsign-core@3.3.1
	reqsign-file-read-tokio@3.0.6
	reqsign-google@3.1.1
	reqsign-huaweicloud-obs@3.0.6
	reqsign-tencent-cos@3.0.6
	reqwest@0.12.28
	reqwest@0.13.5
	reqwest-middleware@0.5.2
	reqwest-retry@0.9.1
	retry-policies@0.5.2
	ring@0.17.14
	roaring@0.11.5
	roxmltree@0.21.1
	rsa@0.9.10
	rstest@0.26.1
	rstest_macros@0.26.1
	rust-ini@0.21.3
	rustc-hash@2.1.3
	rustc_version@0.4.1
	rustix@1.1.5
	rustls@0.23.45
	rustls-native-certs@0.8.4
	rustls-pki-types@1.15.1
	rustls-platform-verifier@0.7.0
	rustls-platform-verifier-android@0.1.1
	rustls-webpki@0.103.15
	rustversion@1.0.23
	ryu@1.0.23
	safe-transmute@0.11.3
	salsa20@0.10.2
	same-file@1.0.6
	schannel@0.1.29
	scopeguard@1.2.0
	scrypt@0.11.0
	security-framework@3.7.0
	security-framework-sys@2.17.0
	semver@1.0.28
	seq-macro@0.3.6
	serde@1.0.229
	serde_core@1.0.229
	serde_derive@1.0.229
	serde_json@1.0.151
	serde_qs@0.8.5
	serde_regex@1.2.0
	serde_repr@0.1.21
	serde_urlencoded@0.7.1
	serial_test@3.5.0
	serial_test_derive@3.5.0
	sha1@0.10.7
	sha1@0.11.0
	sha2@0.10.9
	sha2@0.11.0
	sha2-const-stable@0.1.0
	sharded-slab@0.1.7
	shell-escape@0.1.5
	shellexpand@3.1.2
	shlex@2.0.1
	signal-hook-registry@1.4.8
	signature@2.2.0
	simd-adler32@0.3.10
	simd_cesu8@1.2.0
	simdutf8@0.1.5
	similar@2.7.0
	siphasher@1.0.3
	slab@0.4.12
	small_ctor@0.1.2
	smallvec@1.16.1
	snap@1.1.2
	socket2@0.6.5
	spin@0.9.9
	spki@0.7.3
	sqlparser@0.62.0
	sqlparser_derive@0.5.0
	ssh_format@0.14.2
	ssh_format_error@0.1.0
	stabby@72.1.16
	stabby-abi@72.1.16
	stabby-macros@72.1.16
	stable_deref_trait@1.2.1
	stacker@0.1.25
	static_assertions@1.1.0
	statrs@0.18.0
	stringmetrics@2.2.2
	strsim@0.11.1
	strum@0.28.0
	strum_macros@0.28.0
	subtle@2.6.1
	suppaftp@6.3.0
	symlink@0.1.0
	syn@2.0.119
	syn@3.0.6
	sync_wrapper@1.0.2
	synstructure@0.14.0
	sysinfo@0.38.4
	system-configuration@0.7.0
	system-configuration-sys@0.6.0
	tabwriter@1.4.1
	tagptr@0.2.0
	target-lexicon@0.13.5
	tempfile@3.27.0
	termcolor@1.4.1
	terminal_size@0.4.4
	thin-vec@0.2.20
	thiserror@1.0.69
	thiserror@2.0.20
	thiserror-impl@1.0.69
	thiserror-impl@2.0.20
	thread_local@1.1.10
	tikv-jemalloc-sys@0.7.1+5.3.1-0-g81034ce1f1373e37dc865038e1bc8eeecf559ce8
	tikv-jemallocator@0.7.0
	time@0.3.53
	time-core@0.1.9
	time-macros@0.2.31
	tiny-keccak@2.0.2
	tinystr@0.8.4
	tinytemplate@1.2.1
	tinyvec@1.13.3
	tokio@1.53.1
	tokio-io-utility@0.7.6
	tokio-macros@2.7.2
	tokio-native-tls@0.3.1
	tokio-retry@0.3.2
	tokio-rustls@0.26.5
	tokio-stream@0.1.19
	tokio-test@0.4.5
	tokio-util@0.7.19
	tokio_with_wasm@0.8.8
	tokio_with_wasm_proc@0.8.8
	toml_datetime@1.1.1+spec-1.1.0
	toml_edit@0.25.15+spec-1.1.0
	toml_parser@1.1.3+spec-1.1.0
	tonic@0.14.6
	tonic-prost@0.14.6
	tower@0.5.3
	tower-http@0.6.11
	tower-layer@0.3.3
	tower-service@0.3.3
	tracing@0.1.44
	tracing-appender@0.2.5
	tracing-attributes@0.1.31
	tracing-core@0.1.36
	tracing-log@0.2.0
	tracing-opentelemetry@0.32.1
	tracing-serde@0.2.0
	tracing-subscriber@0.3.23
	triomphe@0.1.16
	try-lock@0.2.5
	twox-hash@2.1.4
	typed-builder@0.23.2
	typed-builder-macro@0.23.2
	typenum@1.20.1
	typewit@1.15.2
	unicode-ident@1.0.26
	unicode-segmentation@1.13.3
	unicode-width@0.2.2
	untrusted@0.9.0
	url@2.5.8
	urlencoding@2.1.3
	utf8_iter@1.0.4
	utf8parse@0.2.2
	uuid@1.26.1
	validator@0.19.0
	validator_derive@0.19.0
	valuable@0.1.1
	value-bag@1.14.1
	vcpkg@0.2.15
	vec-strings@0.4.8
	version_check@0.9.5
	vsimd@0.8.0
	waker-fn@1.2.0
	walkdir@2.5.0
	want@0.3.1
	wasi@0.9.0+wasi-snapshot-preview1
	wasi@0.11.1+wasi-snapshot-preview1
	wasi@0.14.7+wasi-0.2.4
	wasip2@1.0.4+wasi-0.2.12
	wasite@0.1.0
	wasite@1.0.2
	wasm-bindgen@0.2.128
	wasm-bindgen-futures@0.4.78
	wasm-bindgen-macro@0.2.128
	wasm-bindgen-macro-support@0.2.128
	wasm-bindgen-shared@0.2.128
	wasm-streams@0.4.2
	wasm-streams@0.5.0
	wasmtimer@0.4.3
	web-sys@0.3.105
	web-time@1.1.0
	webpki-root-certs@1.0.9
	which@7.0.3
	which@8.0.6
	whoami@1.6.1
	whoami@2.1.3
	winapi@0.3.9
	winapi-i686-pc-windows-gnu@0.4.0
	winapi-util@0.1.11
	winapi-x86_64-pc-windows-gnu@0.4.0
	windows@0.62.2
	windows-collections@0.3.2
	windows-core@0.62.2
	windows-future@0.3.2
	windows-implement@0.60.2
	windows-interface@0.59.3
	windows-link@0.2.1
	windows-numerics@0.3.1
	windows-registry@0.6.1
	windows-result@0.4.1
	windows-strings@0.5.1
	windows-sys@0.52.0
	windows-sys@0.59.0
	windows-sys@0.60.2
	windows-sys@0.61.2
	windows-targets@0.52.6
	windows-targets@0.53.5
	windows-threading@0.2.1
	windows_aarch64_gnullvm@0.52.6
	windows_aarch64_gnullvm@0.53.1
	windows_aarch64_msvc@0.52.6
	windows_aarch64_msvc@0.53.1
	windows_i686_gnu@0.52.6
	windows_i686_gnu@0.53.1
	windows_i686_gnullvm@0.52.6
	windows_i686_gnullvm@0.53.1
	windows_i686_msvc@0.52.6
	windows_i686_msvc@0.53.1
	windows_x86_64_gnu@0.52.6
	windows_x86_64_gnu@0.53.1
	windows_x86_64_gnullvm@0.52.6
	windows_x86_64_gnullvm@0.53.1
	windows_x86_64_msvc@0.52.6
	windows_x86_64_msvc@0.53.1
	winnow@1.0.4
	winsafe@0.0.19
	wit-bindgen@0.57.1
	writeable@0.6.4
	xattr@1.6.1
	xet-client@1.6.0
	xet-core-structures@1.6.0
	xet-data@1.6.0
	xet-runtime@1.6.0
	xmlparser@0.13.6
	yansi@1.0.1
	yoke@0.8.3
	yoke-derive@0.8.3
	z85@3.0.7
	zerocopy@0.8.57
	zerocopy-derive@0.8.57
	zerofrom@0.1.8
	zerofrom-derive@0.1.8
	zeroize@1.9.0
	zerotrie@0.2.5
	zerovec@0.11.8
	zerovec-derive@0.11.6
	zlib-rs@0.6.8
	zmij@1.0.23
	zstd@0.13.3
	zstd@0.14.0
	zstd-safe@7.3.0
	zstd-safe@8.0.0
	zstd-sys@2.1.0+zstd.1.5.7
"

declare -A GIT_CRATES=(
	[buoyant_kernel]="https://github.com/buoyant-data/delta-kernel-rs;${KERNEL_COMMIT};delta-kernel-rs-%commit%/kernel"
	[buoyant_kernel_derive]="https://github.com/buoyant-data/delta-kernel-rs;${KERNEL_COMMIT};delta-kernel-rs-%commit%/derive-macros"
	[buoyant_kernel_engine]="https://github.com/buoyant-data/delta-kernel-rs;${KERNEL_COMMIT};delta-kernel-rs-%commit%/default-engine"
)

# Match the workspace's rust-version. Verified 2026-09-21.
RUST_MIN_VER="1.94.1"

inherit cargo distutils-r1

DESCRIPTION="Native Delta Lake Python binding based on delta-rs"
HOMEPAGE="
	https://github.com/delta-io/delta-rs
	https://pypi.org/project/deltalake/
	https://delta-io.github.io/delta-rs/
"
# PyPI is wheel-only. The monorepo omits Cargo.lock, so CRATES comes from a
# generated workspace lock; see the deltalake memory for the recipe.
SRC_URI="
	https://github.com/delta-io/delta-rs/archive/refs/tags/python-v${PV}.tar.gz -> ${P}.gh.tar.gz
	${CARGO_CRATE_URIS}
"
S="${WORKDIR}/delta-rs-python-v${PV}/python"

LICENSE="Apache-2.0"
LICENSE+="
	Apache-2.0 Apache-2.0-with-LLVM-exceptions BSD-2 BSD CC0-1.0
	CDLA-Permissive-2.0 ISC MIT MPL-2.0 Unicode-3.0 ZLIB BZIP2
"
SLOT="0"
KEYWORDS="~amd64 ~arm64"

RDEPEND="
	>=dev-python/arro3-core-0.5.0[${PYTHON_USEDEP}]
	>=dev-python/deprecated-1.2.18[${PYTHON_USEDEP}]
"

# Tests need network access to localstack/azure/gcp emulators.
RESTRICT="test"

src_prepare() {
	local ws_toml="${WORKDIR}/delta-rs-python-v${PV}/Cargo.toml"
	local kernel_dir="${WORKDIR}/delta-kernel-rs-${KERNEL_COMMIT}"
	local pin='git = "https://github.com/buoyant-data/delta-kernel-rs", rev = "'${KERNEL_COMMIT}'"'

	grep -qF "delta_kernel = { package = \"buoyant_kernel\", ${pin}" \
		"${ws_toml}" || die "delta_kernel pin changed -- re-audit"
	grep -qF "delta_kernel_default_engine = {  package = \"buoyant_kernel_engine\",  ${pin}" \
		"${ws_toml}" || die "delta_kernel_default_engine pin changed -- re-audit"

	sed -i \
		-e "/^delta_kernel = / s|${pin}|path = \"${kernel_dir}/kernel\"|" \
		-e "/^delta_kernel_default_engine = / s|${pin}|path = \"${kernel_dir}/default-engine\"|" \
			"${ws_toml}" || die

	distutils-r1_src_prepare
}

python_compile() {
	local profile=python-release
	use arm64 && profile+="-arm"
	local DISTUTILS_ARGS=( --profile "${profile}" )

	distutils-r1_python_compile
}
