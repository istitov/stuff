...
# keep the runtime toolchain explicit. blk128 imports quack despite omitting it
# from requirements. >=nvidia-cutlass-dsl-4.6.2 satisfies upstream's 4.6.2a0
# floor; excluded apache-tvm-ffi 0.1.8 versions predate this overlay's minimum.
# requirements rechecked against 0.7.0rc3 on 2026-09-19
RDEPEND="
	...
	$(python_gen_cond_dep '
		...
		>=dev-python/nvidia-cudnn-frontend-1.29.0[${PYTHON_USEDEP}]
...
src_prepare() {
	...
	# Transport backends are disabled below and imported lazily; do not require
	# their unpackaged Python frontends for the supported JIT kernel library.
	grep -qx 'nccl-extensions>=0.1.0' requirements.txt || die
	grep -qx 'nccl4py>=0.4.1' requirements.txt || die
	sed -e '/^nccl-extensions>=0\.1\.0$/d' \
		-e '/^nccl4py>=0\.4\.1$/d' -i requirements.txt || die
