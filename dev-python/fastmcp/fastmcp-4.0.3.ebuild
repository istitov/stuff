...
#
# None of 4.0.1 / 4.0.2 / 4.0.3 moves any of this: the base, mcp, client and
# server groups in fastmcp_slim/pyproject.toml are unchanged since 4.0.0 -- for
# 4.0.3 both the root and the fastmcp_slim pyproject.toml are byte-identical to
# 4.0.2's. Note that the root package's own PyPI Requires-Dist is useless for
# auditing this -- it lists only `fastmcp-slim[...]==${PV}` pins, so a diff of it
# shows every entry "changing" between releases while the real dependency set is
# untouched. Always read the bundled fastmcp_slim pyproject instead.
# verified 2026-09-05.
RDEPEND="
