...
#
# 4.0.1 changes nothing here: fastmcp_slim/pyproject.toml is byte-identical to
# 4.0.0's. Note that the root package's own PyPI Requires-Dist is useless for
# auditing this -- it lists only `fastmcp-slim[...]==${PV}` pins, so a diff of
# it shows every entry "changing" from 4.0.0 to 4.0.1 while the real dependency
# set is untouched. Always diff the bundled fastmcp_slim pyproject instead.
# verified 2026-09-02.
RDEPEND="
