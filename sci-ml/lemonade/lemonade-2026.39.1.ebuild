...
inherit cmake desktop python-any-r1 systemd
...
src_prepare() {
	...
	# CMake takes the version from tools/version.py, which reads .version before
	# git state. The release archive has neither, so the script would report a
	# build-date "nogit" version; pin the packaged one. verified 2026-09-24
	echo "${PV}" > .version || die
