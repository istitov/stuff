...
# (therock.version), still 7.14.0 at 11.9.0 -- the 7.14 series is exactly
...
# bounds from that json on every bump. verified 2026-09-02 against 11.9.0
# No app-arch/brotli: upstream FetchContent-declares and links brotli only
# inside `if(APPLE)` blocks (CMakeLists.txt:330 and :421) -- httplib needs it
# for compression on macOS only. Nothing on the Linux path uses it.
# zstd carries upstream's floor: CMakeLists.txt sets MIN_ZSTD_VERSION 1.5.5.
# verified 2026-09-02 against the 11.9.0 tarball.
RDEPEND="
	>=app-arch/zstd-1.5.5:=
	>=dev-cpp/cli11-2.4.2
...
src_prepare() {
	...
	# FetchContents cpp-httplib (MIN_HTTPLIB_VERSION 0.26.0). That tag's own
	# CMakeLists.txt has a malformed
	# `if(CMAKE_SYSTEM_NAME MATCHES "Windows" AND VERSION_LESS 10.0.0)`
	# which CMake 4 rejects ("Unknown arguments specified"). Repin the fetch
	# to v0.38.0 — the CMake-4-clean version we ship as dev-cpp/cpp-httplib,
	# API-compatible with the >=0.26.0 floor lemonade actually needs.
	#
	# 11.0.0 writes the httplib FetchContent_Declare as a bare commit SHA
	# (`GIT_TAG 89c932f… # v0.26.0`) rather than 10.x's stable
	# `GIT_TAG v${MIN_HTTPLIB_VERSION}` — so rewrite the declare block's
	# GIT_TAG whatever its form (range match on the block) instead of a fixed
	# string, matching the -9999 ebuild. 11.6.0 moved the pin to a v0.47.0 SHA
	# and added a detect_system_httplib() macro that may resolve ::gentoo's
	# cpp-httplib directly (making this FetchContent path a fallback); the range
	# sed still matches either way. 11.9.0 still carries the v0.47.0 SHA pin
	# with the FetchContent block unchanged, and the sed was re-checked to
	# rewrite exactly one line. verified 2026-09-02 against 11.9.0
	sed -i \
...
src_configure() {
	...
	local mycmakeargs=(
		...
		# 11.6.0 documents BUILD_TESTING=OFF as the distro-packaging path: it
		# skips ~40 test binaries we never run or ship. Upstream's test blocks
		# are all if(BUILD_TESTING ...)-guarded, and add_cpp_ci_test() fatal-
		# errors on an unguarded test rather than silently building it.
		-DBUILD_TESTING=OFF
	)
	cmake_src_configure
}
...
src_install() {
	...
	# sysusers.d snippet, a secrets EnvironmentFile, and a migrate-to-systemd
	# helper. Gate the lot behind USE=systemd -- the system unit runs as
	# User=lemonade (provided by acct-user/lemonade).
	# Without USE=systemd, strip all of it so OpenRC (or unit-less) installs
	# don't carry dead systemd files. The secrets file especially is worse than
	# dead on OpenRC: the init reads /etc/conf.d/lemonade, so an API key
	# dropped in the systemd EnvironmentFile is silently ignored (no auth)
	# rather than applied. sysusers.d/lemonade.conf is redundant with the
	# unconditional acct-user/lemonade; keep it under systemd (idempotent),
	# drop it here.
	#
	# 11.8.0 moved that EnvironmentFile from /etc/lemonade/conf.d/zz-secrets.conf
	# to /etc/default/lemond (unit switched to EnvironmentFile=-/etc/default/lemond,
	# mode 0660 -> 0640), so the /etc/lemonade tree is no longer created at all.
	# Never rmdir /etc/default -- it is a shared system directory.
	#
	# The paths below mirror exactly what upstream's CMake installs as of
	# v11.0.0. On a bump, re-confirm each basename/dir -- a rename upstream
	# turns any of these `rm ... || die` into a build failure. NB: since
	# v11.0.0 the examples/ dir also carries non-systemd API samples, so the
	# rmdir below no-ops (dir non-empty) -- only migrate-to-systemd.sh is
	# systemd-specific and stripped; the samples ship regardless of USE.
	# verified 2026-08-27 against 11.8.0
	if ! use systemd; then
	...
	# therock.version 7.14 -- the RDEPEND holds it to the 7.14 line). OpenRC
