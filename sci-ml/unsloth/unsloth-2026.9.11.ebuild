...
# matches ${PV}. GitHub archives are mutable, so review any Manifest rehash.
# Verified 2026-09-23.
MY_TAG="v0.1.815-beta"
...
RDEPEND="
	>=dev-python/unsloth-zoo-2026.9.7[${PYTHON_SINGLE_USEDEP}]
	...
	')
	studio? (
		>=sci-ml/huggingface_hub-1.31[${PYTHON_SINGLE_USEDEP}]
		$(python_gen_cond_dep '
			...
			>=dev-python/av-12.3[${PYTHON_USEDEP}]
...
src_prepare() {
	...
	# Version-named since 2026.9.8: upstream reflowed the comments around both
	# call sites there, so the copy older versions carry no longer applies.
	use studio && PATCHES+=( "${FILESDIR}/${P}-system-studio.patch" )

	# find_packages includes 88 KiB of MoE tests absent from upstream's wheel.
	# Verified 2026-09-10.
