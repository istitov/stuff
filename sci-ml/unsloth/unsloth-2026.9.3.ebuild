...
# whose desktop tags are full snapshots -- v0.1.807-beta declares library
# version 2026.9.3 in unsloth/_version.py.  The tag/version map moves in
# lockstep, so a bump picks the tag whose _version.py matches ${PV}.
# 2026.9.3 has no sdist at all, so the tag is the only source.
#
...
RDEPEND="
	>=dev-python/unsloth-zoo-2026.9.2[${PYTHON_SINGLE_USEDEP}]
...
src_prepare() {
	...
	# that upstream's own 2026.9.3 wheel does not ship. Drop it so the
	# installed set matches the published one. # verified 2026-09-09
