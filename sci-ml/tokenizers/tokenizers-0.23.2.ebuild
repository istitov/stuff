...
# Diverges from ::gentoo on purpose: that ebuild carries an empty CRATES and
# pulls two pre-bundled crate archives from dev.gentoo.org/~tupone, which were
# never published for 0.23.2 (both 404). CRATES is regenerated from
# both Cargo.locks instead, so the crates come from crates.io like
# any other Rust package here and the bump does not wait on a third party.
# The ebuild builds two crate graphs -- tokenizers/ and bindings/python/ --
# and only the latter ships a lock, so the core one is generated with
# `cargo generate-lockfile` first and the two crate sets unioned.
# Re-run pycargoebuild on both when bumping. # verified 2026-09-09
