...
# Core install_requires per setup.py (verified against v0.20.1 sdist).
...
#
# python-multipart — v0.20.1 promotes it from the api/ui extras into the
# base install_requires: the base `gaia-mcp` console_script parses
# multipart uploads via python_multipart at import time, so a plain
# install needs it even without USE=api (latent gap in 0.20.0, where
# gaia-mcp was already a base console_script but the dep was extras-only).
# Carried in base here and dropped from api? as redundant. verified
# 2026-06-11 against v0.20.1.
#
# audio? — gaia code only `import torch`s (gaia/audio/whisper_asr.py:19);
# never imports torchvision OR torchaudio (re-grepped src/ for v0.20.1,
# 2026-06-11). Upstream's `audio` extra caps torch<2.13 because of
...
# overlay yet; revisit once it lands. still present in v0.20.1, verified
# 2026-06-11.
RDEPEND="
	${PYTHON_DEPS}
