...
# Core install_requires per setup.py (verified against v0.23.0 sdist
# 2026-08-14).
#
# cryptography — v0.23.0 promotes it to base install_requires (>=42.0.0,
# upstream #2467): `gaia skill install` verifies an Ed25519 signature at the
# core level (a skill's security tier rests on its signature), so it is not an
# extra. verified 2026-08-14.
#
# cvss — v0.23.0 adds cvss>=3.6,<4.0 to the eval extra for the security-audit
# workflow (util/cvss4.py, util/findings_to_sarif.py; lazy CLI-invoked, not an
# eval-runtime import). Not packaged in ::gentoo or here, so omitted — the CVSS
# scoring sub-feature degrades, like tavily-python above. verified 2026-08-14.
#
# pywin32 — v0.23.0 adds it for a win32 owner-only DACL on the sidecar secret
# (sys_platform == "win32" only); irrelevant on ~amd64, so not carried.
...
#
# httpx (api?, ui?) — hard upstream req. The ui extra declares httpx>=0.27.0
# (9 src/gaia modules import it); v0.23.0 also adds it to the api extra: the
# API server is now the sole path to agent surfaces (#2176/#2178) and streams
# SSE from the daemon relay over async httpx (no agent mounts in-process).
# ::gentoo-deprecated 2026-04-01 (no drop-in replacement), so the DeprecatedDep
# warning is knowingly accepted. verified 2026-06-20; api addition 2026-08-14.
#
