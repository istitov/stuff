...
# Upstream lists golang.org/x/sync twice in go.mod, which makes every
# -mod=vendor build refuse to start. verified 2026-09-22
PATCHES=( "${FILESDIR}/${P}-go-mod-duplicate-require.patch" )
...
src_compile() {
	...
	# !embed_ui supplies the no-UI implementation. At 257 the lockfile, output,
	# tag pair, and main.version ldflag target remain valid. The new
	# cmd/kubeswap binary is not built here. # verified 2026-09-22
	local build_tags=()
	if use ui; then
