...
# verified against 1.122 on 2026-09-26
RDEPEND="
	${PYTHON_DEPS}
	$(python_gen_cond_dep '
...
src_install() {
	...
	insinto "${dest}"
	# --agent loads the stdlib-only kcpp_agent.py from beside the launcher.
	doins koboldcpp.py kcpp_agent.py
	...
	python_fix_shebang "${ED}${dest}"/{koboldcpp,kcpp_agent}.py
