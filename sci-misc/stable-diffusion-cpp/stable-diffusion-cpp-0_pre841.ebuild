...
# The vendored ggml has no GGML_HIP_ROCWMMA_FATTN option; the wmma USE flag was
# dropped at 0_pre829 and stays dropped. The gitlink MOVED for this bump
# (8e800cef -> e20c3a14), so 0_pre829's check does not carry over and this was
# re-verified 2026-08-30 against the unpacked tree at this ebuild's own
# GGML_COMMIT: zero GGML_HIP_ROCWMMA_FATTN occurrences, and the sole rocWMMA
# mention anywhere in the tree is a "TODO add support for AMD cards via
# rocWMMA" comment in ggml-cuda/lightning-indexer.cu, whose actual include is
# nvcuda::wmma via <mma.h> -- NVIDIA-only, carrying no rocWMMA dependency. The
# ggml option() surface is otherwise identical across the two gitlinks.
#
# Matches sci-misc/llama-cpp: upstream removed the option in b10121 (PR #26046,
# "HIP: remove rocWMMA FlashAttention") and HIP FlashAttention now runs through
# the shared ggml-cuda fattn-mma templates. Keeping the flag would have pulled
# in sci-libs/rocWMMA, which ::gentoo ships only up to 7.2.0 -- pinning the
# ROCm stack at 0/7.2 and making the 10.0 closure unsolvable.
