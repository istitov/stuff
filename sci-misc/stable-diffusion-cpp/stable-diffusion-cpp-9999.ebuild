...
# No GGML_HIP_ROCWMMA_FATTN option in the vendored ggml. Verified 2026-08-30
# against the release ebuilds' pinned GGML_COMMIT 8e800cef -- zero hits in the
# source, the only occurrences in the build being the -D this ebuild passed and
# the environment derived from it. Not re-verified against live master here,
# but the option is gone upstream, so a live checkout cannot have it either.
#
# CMake silently ignores an unknown -D, so the flag changed nothing about the
# build while still pulling in sci-libs/rocWMMA -- which ::gentoo ships only up
# to 7.2.0, so it pinned the ROCm stack at 0/7.2 and made the 10.0 closure
# unsolvable. Dropped, following sci-misc/llama-cpp: upstream removed the same
