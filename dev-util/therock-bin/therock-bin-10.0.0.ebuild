...
# AMD publishes only a Makeself runfile; release assets otherwise contain source.
# Extract its shared base and family payload without running the installer;
# $ORIGIN rpaths work under /opt/therock-bin. BUILDINFO says 10.0.0rc4, while AMD
# labels it 10.0.0. MY_BUILD tracks reissued installers.
MY_BUILD="4"
...
src_install() {
	...
	[[ ${found} ]] || die "no rocm/${core} component trees found for ${arch}"
	fperms 0755 "${dest}"
}
