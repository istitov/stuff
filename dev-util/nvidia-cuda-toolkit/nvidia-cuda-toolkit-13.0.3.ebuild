...
cuda_verify_compilers() {
	...
	GCC_HAS_VER="$(sed -n -E 's/.*gcc versions later than ([0-9]+).*/\1/p' "${host_config[@]}")"
	...
	CLANG_HAS_VER="$(sed -n -E 's/.*clang version must be less than ([0-9]+).*/\1/p' "${host_config[@]}")"
