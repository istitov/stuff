...
# Map 7_preN to upstream v7-preview.N.
...
# Installed cdylibs directly link HIP, rocm-smi, hipFFT, rocBLAS, hipBLASLt,
# and rocSPARSE; := forces rebuilds across ROCm SONAME changes. cuFFT went from
# a stub to hipFFT in 7_pre11. LZ4 and LLVM are bundled; amd-comgr is no longer
# linked. verified 2026-09-22
RDEPEND="
	dev-util/hip:=
	dev-util/rocm-smi:=
	sci-libs/hipFFT:=
...
pkg_postinst() {
	elog ""
	elog "ZLUDA ${PV} (upstream tag v${PV/_pre/-preview.}) installed under /opt/zluda/."
	elog ""
	...
	elog "preview-tagged builds are upstream's recommended consumer artifact."
	elog ""
	elog "For the rolling tip, use =dev-util/zluda-9999 instead."
}
