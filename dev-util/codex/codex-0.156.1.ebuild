...
# Use gentoo-zh-drafts' crate tarball: 1,319 CRATES entries exceed pkgcheck's
...
# 0.156.1: all 1,322 crates and the Cargo.lock set matched. RUSTY_V8_TAG=150.4.0
# was re-derived from the lock and the upstream script, and the lock adds
# h3/h3-quinn as git dependencies, so GIT_CRATES grows by two.
# Verified 2026-09-23.
# pycargoebuild rejects workspace roots; self-hosting ~150 MiB per release is
...
# rustc 1.98 aborts on the chatgpt crate's query depth; upstream pins 1.95.
PATCHES=( "${FILESDIR}/${P}-chatgpt-recursion-limit.patch" )
