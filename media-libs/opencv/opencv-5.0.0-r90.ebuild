...
# modules
# NOTE OpenCV 5.x moved the ml and gapi (G-API) modules into opencv_contrib, so
# they are now only built (and only exposed to python) when USE=contrib is set.
...
# image
# quirc dropped: 5.x has its own QR decoder and no longer uses media-libs/quirc.
# jpegxl is new in 5.x (WITH_JPEGXL).
...
PATCHES=(
	...
	# rebased from gentoo's 4.8.1/4.9.0 patches; still apply to the 5.0.0 tree.
	# use-system-opencl must come after opencl-license (which drops its context line).
	...
	#                                 to the generic-LAPACK link line itself
	#   drop-python2-detection      - python2 support removed upstream
	#   cmake-4                      - gapi's cmake_minimum_required is already 3.13
	# The ADE tar.gz patch moved to the contrib block below (gapi is contrib in 5.x).
	# (earlier-dropped at 4.13.0: protobuf-34, qt-6.9, cuda-13.0, ffmpeg8,
	# fix_videowriter_raw_return_code)

	# TODO applied in src_prepare (contrib)
	# "${FILESDIR}/${PN}_contrib-4.8.1-rgbd.patch"

	# "${FILESDIR}/${PN}_contrib-4.8.1-NVIDIAOpticalFlowSDK-2.0.tar.gz.patch"

	# OpenCV 5.0.0 carries the CUDA 13.2+ fixes natively, so neither the cudev
	# zip.hpp namespace patch nor the cudacodec cuda-include patch is needed.
)
...
src_unpack() {
	...
	local files_3rdparty=(
		...
		# ittnotify
		libjasper
		# 5.x no longer bundles plain libjpeg, openexr or quirc (system/FetchContent)
		libjpeg-turbo
		libpng
		libspng
		libtiff
...
src_prepare() {
	...
	if use contrib; then
		...
		# OpenCV 5.x moved gapi (G-API) into opencv_contrib.  Point its ADE download
		# at the github tar.gz mirror we ship as a distfile instead of the upstream
		# .zip (the patch's md5 matches contrib's DownloadADE.cmake unchanged).
	...
	if [[ ${PV} = *9999* ]] && use contrib ; then
		# gapi (and its DownloadADE.cmake) lives in opencv_contrib for 5.x.
		sed \
...
multilib_src_configure() {
	...
	# ===================================================
		...
		-DWITH_V4L="$(usex v4l)"
		# 5.x v4l uses the kernel videodev2.h directly (no libv4l); WITH_LIBV4L is gone.
		# WITH_OBSENSOR (Orbbec depth cams) defaults ON and builds a V4L2 backend; pin it.
		-DWITH_OBSENSOR="$(usex v4l)"
		...
		# WITH_FREETYPE moved to the contrib block (it is a gapi option in 5.x)
...
multilib_src_install() {
	if use abi_x86_64 && use abi_x86_32; then
		MULTILIB_WRAPPED_HEADERS=( # {{{
			...
			# (mcc module was removed upstream in 5.0.0)
