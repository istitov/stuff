...
if [[ ${PV} == *9999* ]] ; then
	...
	"
	# Runtime-verified on amd64; install-verified with ~amd64 and ~arm64.
...
# modules; OpenCV 5 moved ml and gapi to opencv_contrib
...
# image
# OpenCV 5 replaces quirc with its own decoder and adds WITH_JPEGXL.
...
PATCHES=(
	...
	# Rebases of Gentoo patches; use-system-opencl depends on opencl-license's edit.
	...
	# Restore the dropped Caffe schema and regenerate it with system protobuf;
	# upstream's stale protoc-3.19 output no longer compiles. istitov/stuff#271;
	# contribdnn build verified 2026-06-07.
...
src_prepare() {
	...
	# Restore the compressed 4.13 schema; it regenerates byte-identical output.
	...
	if use contrib; then
		...
		# Redirect contrib's ADE download to the mirrored tarball.
		...
		# ADE belongs to contrib's gapi module in OpenCV 5.
	...
	if [[ ${PV} = *9999* ]] && use contrib ; then
		# Adjust the live contrib checkout for the mirrored ADE tarball.
		sed \
...
multilib_src_configure() {
	...
	# Optional third-party components
		...
		-DWITH_V4L="$(usex v4l)"
		# WITH_OBSENSOR defaults on and supplies a V4L2 depth-camera backend.
	...
	# Modules
		...
		# gapi moved to contrib and includes features.hpp without declaring the module;
		# gate it explicitly to avoid a missing header. istitov/stuff#271;
		# verified 2026-06-08
		-DBUILD_opencv_gapi="$(usex contrib "$(usex features "$(usex ffmpeg yes "$(usex gstreamer)")" no)" no)"
		-DBUILD_opencv_ml="$(usex contrib)"
		# features2d was renamed to features in OpenCV 5.
...
pkg_postinst() {
	# Warn users whose old features2d setting silently stopped applying.
