...
# @PRE_INHERIT
# @DESCRIPTION:
# When set, fetch a KDE Qt5 Patch Collection bundle and apply it in
# src_prepare. The value's format dispatches the source:
#
# - "rN-bump" (e.g. "r0-0", "r0-1") -> stuff overlay's extra-stuff sister
#   repo. Bundle is staged at dev-qt/${QT5_MODULE}/${QT5_MODULE}-${PV}-kde.tar.xz
#   under tag ${QT5_MODULE}-${PV}-kde-${QT5_KDEPATCHSET_REV}, self-cut via:
#   @CODE
#   git format-patch v${PV}-lts-lgpl..origin/kde/5.15 \
#       -o ${QT5_MODULE}-${PV}-kde
#   @CODE
#   from https://invent.kde.org/qt/qt/${QT5_MODULE}
#
# - Numeric (e.g. "1") -> legacy gentoo-cut patchset from asturm's distfile.
#   Retained for transitional 5.15.18 ebuilds (qthelp, qtwebchannel) until
#   they bump to 5.15.19. Will be removed when no consumers remain.
#
...
# qtbase build-system patchset (originally gentoo's, now mirrored to extra-stuff
# under a stable name). Always applied for QT5_MODULE=qtbase. No per-ebuild knob;
# bumps are managed via tag revision on extra-stuff.
	...
	case ${PV} in
		*9999)
		...
		5.15.??*)
			...
			# KDE Qt5 Patch Collection on top of tag v${PV}-lts-lgpl.
			# New (rN-bump) -> stuff overlay's extra-stuff sister repo, self-cut.
			# Legacy (numeric) -> kept for 5.15.18 ebuilds during transition.
			if [[ -n ${QT5_KDEPATCHSET_REV} ]]; then
				HOMEPAGE+=" https://invent.kde.org/qt/qt/${QT5_MODULE} https://community.kde.org/Qt5PatchCollection"
				if [[ ${QT5_KDEPATCHSET_REV} =~ ^r[0-9]+-[0-9]+$ ]]; then
					_QT5_KDE_TAG="${QT5_MODULE}-${PV}-kde-${QT5_KDEPATCHSET_REV}"
					_QT5_KDE_PATH="dev-qt/${QT5_MODULE}/${QT5_MODULE}-${PV}-kde.tar.xz"
					_QT5_KDE_DISTFILE="${QT5_MODULE}-${PV}-kde-${QT5_KDEPATCHSET_REV}.tar.xz"
					SRC_URI+="
						https://raw.githubusercontent.com/istitov/extra-stuff/${_QT5_KDE_TAG}/${_QT5_KDE_PATH} -> ${_QT5_KDE_DISTFILE}
						https://codeberg.org/istitov/extra-stuff/raw/tag/${_QT5_KDE_TAG}/${_QT5_KDE_PATH} -> ${_QT5_KDE_DISTFILE}
						https://gitlab.com/istitov/extra-stuff/-/raw/${_QT5_KDE_TAG}/${_QT5_KDE_PATH} -> ${_QT5_KDE_DISTFILE}
					"
				else
...
if [[ ${QT5_MODULE} == qtbase ]]; then
	_QT5_GP_TAG="qtbase-5.15-gentoo-patchset-r0-0"
	_QT5_GP_PATH="dev-qt/qtbase/qtbase-5.15-gentoo-patchset.tar.xz"
	_QT5_GP_DISTFILE="qtbase-5.15-gentoo-patchset-r0-0.tar.xz"
	SRC_URI+="
		https://raw.githubusercontent.com/istitov/extra-stuff/${_QT5_GP_TAG}/${_QT5_GP_PATH} -> ${_QT5_GP_DISTFILE}
		https://codeberg.org/istitov/extra-stuff/raw/tag/${_QT5_GP_TAG}/${_QT5_GP_PATH} -> ${_QT5_GP_DISTFILE}
		https://gitlab.com/istitov/extra-stuff/-/raw/${_QT5_GP_TAG}/${_QT5_GP_PATH} -> ${_QT5_GP_DISTFILE}
	"
fi
...
qt5-build_src_prepare() {
	...
	if [[ -n ${QT5_KDEPATCHSET_REV} ]]; then
		if [[ ${QT5_KDEPATCHSET_REV} =~ ^r[0-9]+-[0-9]+$ ]]; then
			eapply "${WORKDIR}/${QT5_MODULE}-${PV}-kde"
		else
			eapply "${WORKDIR}/${QT5_MODULE}-${PV}-gentoo-kde-${QT5_KDEPATCHSET_REV}"
		fi
	fi

	if [[ ${QT5_MODULE} == qtbase ]]; then
		eapply "${WORKDIR}/qtbase-5.15-gentoo-patchset"
